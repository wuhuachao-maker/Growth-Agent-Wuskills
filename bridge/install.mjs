 ...
 * hermes / kiro / qwen / cline / grok / all。
 *
 * 用法：
 *   node bridge/install.mjs --target workbuddy [--dry-run] [--force] [--uninstall] [--scope user|project]
 */
...
for (let i = 0; i < args.length; i++) {
  if (args[i] === "--target") TARGET = args[++i] || "";
    ...
    console.log("用法: node bridge/install.mjs --target <name> [--dry-run] [--force] [--uninstall] [--scope user|project]");
    process.exit(0);
  }
}
...
const resolveDir = (t) => {
  switch (t) {
    ...
    case "creator-upload": return path.join(REPO_DIR, "dist/upload");
    default:
      console.error(`❌ 不支持的 target: ${t}（支持: workbuddy|claude|codex|cursor|agents|hermes|kiro|qwen|cline|grok|all）`);
      process.exit(1);
  }
};
...
if (UNINSTALL) console.log("🗑 uninstall 模式：将删除已安装的同名 skill");
console.log("---------------------------------------------------");

function packageUploadBundle() {
  const dest = path.join(REPO_DIR, "dist/upload");
...
function uninstallOne(t) {
  ...
  for (const name of fs.readdirSync(SKILLS_DIR)) {
    ...
    if (!fs.existsSync(d)) {
      console.log(`  ⏭  未安装: ${name}`);
      continue;
    }
    if (DRY_RUN) {
      console.log(`  - 将删除: ${name} → ${d}`);
    } else {
  ...
  console.log(`  ✅ ${t}: ${count} 个 skill 已卸载`);
}

function installOne(t) {
  if (t === "creator-upload") { packageUploadBundle(); return; }
  if (!clientInstalled(t)) {
    console.log(`  ⏭  ${t}: 对应客户端未安装,跳过`);
    return;
  }
